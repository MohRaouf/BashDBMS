...
then
 rm -r DBMS/$name;
 zenity --info --width=300  --title="Drop Database Status" --text="$name Database Droped Successfully"
 if [ $? -eq 0 ]
 then
   . DBMS.sh
 fi
else
  zenity --error --width=300  --title="Drop Database Status" --text="$name Database Not Exist"
  if [ $? -eq 0 ]
  then
    . DBMS.sh
  fi
