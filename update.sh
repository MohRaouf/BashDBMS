...
line_no=$(awk 'BEGIN{FS="|"; found=0} {if(NR!=1){for(i=1;i<=NF;i++){if($i=="'$selected_record'")found=NR}}} END{print found}' "$table_name")
...
if ((is_pk == "pk")); then
        value_exist=$(awk 'BEGIN{FS="|"; found=0} {if(NR!=1 && NR !="'$line_no'"){for(i=1;i<=NF;i++){if($i=="'$new_value'")found=1}}} END{print found}' "$table_name")
...
awk -v new_value="$new_value" -i inplace -F'|' '{OFS=FS}{if (NR=="'$line_no'"){ $"'$field_no'"=new_value } print}' "$table_name" 2>../../error.log
